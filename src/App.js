...
export default function App() {
  ...
  const [bookmarkedAll, setBookmarkedAll] = useState([]);
  useEffect(() => {
    setMovies(streams.filter((item) => item.category === "Movie"));
    setTvSeries(streams.filter((item) => item.category === "TV Series"));
    setBookmarkedMovies(
      streams.filter((item) => item.isBookmarked && item.category === "Movie")
    );
    setBookmarkedSeries(
      streams.filter(
    ...
    setBookmarkedAll(streams.filter((item) => item.isBookmarked));
  }, [streams]);

  const bookmarkStream = (stream) => {
    setStreams(
      [...streams].map((elem) => {
        if (elem.title === stream.title) {
          return {
            ...elem,
            isBookmarked: !elem.isBookmarked
          };
        } else return elem;
      })
    );
